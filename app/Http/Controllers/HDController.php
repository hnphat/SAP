...
//        ]);
        ...
        if ($hasPK) {
            ...
            foreach($package as $row) {
                ...
                if ($row->type == 'pay') {
                    ...
                    $sumpk += $row->cost - ($row->cost*$saleOff->giamGia/100);
        ...
        $templateProcessor = new TemplateProcessor('template/DENGHI.docx');
            ...
            foreach($package as $row) {
                ...
                if ($row->type == 'pay') {
                    ...
                    $tongPhuKien += $row->cost - ($row->cost*$saleOff->giamGia/100);
                    $dspk .=  $row->name . ";";
                    $dem++;
                    $sttPKB .= $j . '<w:br/>';

                    $pkpay .=  $row->name . " " .($saleOff->giamGia != 0 ? "(-".$saleOff->giamGia."%)" : ""). '<w:br/>';
    ...
    public function taoMau(Request $request) {
        ...
        if($code) {
            ...
            $nhatKy->noiDung = "Thực hiện khởi tạo hợp đồng mới. Danh mục: Phí trước bạ; Phí đăng ký xe; Phí đăng kiểm xe; Phí đường bộ; Bảo hiểm TNDS; Bảo hiểm vật chất; Hỗ trợ đăng ký - đăng kiểm; Chi phí khác";
            $nhatKy->save();
    ...
    public function getpkpay($id) {
        $pkban = SaleOffV2::select('package.*','saleoffv2.giamGia')
        ->join('packagev2 as package','saleoffv2.id_bh_pk_package','=','package.id')
    ...
    public function postEditPKCost(Request $request){
        ...
        if ($check->admin_check != 1 && $check->requestCheck == false) {
            ...
            if($pkpay) {
                ...
                . number_format(round($temppkpay->cost,2)) . " thành giá: " . number_format(round($request->egiapk,2));
                $nhatKy->save();
                ...
                . number_format(round($temppkpay->cost,2)) . " thành giá: " . number_format(round($request->egiapk,2)) . "<br/>Tặng: " .($request->etang ? "Có" : "Không");
                $his->ghiChu = "";
                $his->save();
    ...
    public function postEditPKFree(Request $request){
        ...
        if ($check->admin_check != 1 && $check->requestCheck == false) {
            ...
            if($pkpay) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Quản lý đề nghị";
                $nhatKy->noiDung = "Điều chỉnh nội dung khuyến mãi, quà tặng cho đề nghị ĐN/0".$request->idSaleHDFree
                ."(không phải mã hợp đồng) <br/>Nội dung: " . $request->ndfree 
                . "Từ giá: "
        ...
        if ($check->lead_check != 1) {
            ...
            if($pkpay) {
                ...
                if($saleOff) {
                    ...
                    $nhatKy->thoiGian = Date("H:m:s");
                    $nhatKy->ghiChu = Carbon::now();
                    $nhatKy->noiDung = "Thêm phụ kiện bán cho đề nghị ĐN/0".$request->idHD."(không phải mã hợp đồng) <br/>Nội dung: " . $request->namePkPay
                    ...
                    $his->id_user = Auth::user()->id;
                    $his->ngay = Date("H:m:s d-m-Y");
                    $his->noiDung = "Thêm phụ kiện bán <br/>Nội dung: " . $request->namePkPay . " <br/>Giá: "
    ...
    public function deletePkPay(Request $request) {
        ...
        if ($check->lead_check != 1 && $check->requestCheck != 1) {
            ...
            if($result) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Quản lý đề nghị";
                $nhatKy->noiDung = "Xóa phụ kiện bán cho đề nghị ĐN/0".$request->sale."(không phải mã hợp đồng) <br/>Nội dung: " . $tempac->name . " <br/>Giá: " . number_format(round($tempac->cost,2));
                $nhatKy->save();
                ...
                $his->id_user = Auth::user()->id;
                $his->ngay = Date("H:m:s d-m-Y");
                $his->noiDung = "Xóa phụ kiện bán <br/>Nội dung: " . $tempac->name . " <br/>Giá: " . number_format(round($tempac->cost,2));
                $his->ghiChu = "";
                $his->save();
    ...
    public function deletePkFree(Request $request) {
        ...
        if ($check->lead_check != 1 && $check->requestCheck != 1) {
            ...
            if($result) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Quản lý đề nghị";
                $nhatKy->noiDung = "Xóa phụ kiện tặng cho đề nghị ĐN/0".$request->sale."(không phải mã hợp đồng) <br/>Nội dung: " . $tempac->name . " <br/>Giá: " . round($tempac->cost,2);
                $nhatKy->save();
    ...
    public function xoaDeNghi(Request $request){
        ...
        if($result->admin_check == false && $result->lead_check == false) {
            ...
            $delHis = HistoryHopDong::where('idDeNghi',$request->id)->delete();
            $result->delete();
            if($result) {             
                $nhatKy = new NhatKy();
                $nhatKy->id_user = Auth::user()->id;
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Quản lý đề nghị";
                $nhatKy->noiDung = "Xoá đề nghị ĐN/0".$request->id;
                $nhatKy->save();
                return response()->json([
                    'type' => 'success',
                    'message' => 'Đã xóa!',
    ...
    public function ganXeHDCho(Request $request){
        ...
        if((Auth::user()->hasRole('adminsale') || Auth::user()->hasRole('system')) && $result->requestCheck == true && $result->admin_check == true && $result->lead_check == true) {
            ...
            if($result) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                $nhatKy->noiDung = "Gán xe: ".$car->typeCarDetail->name." VIN: " .$sokhung. " màu: ".$mau." cho hợp đồng chờ số ".$temp. " (Mã đề nghị ĐN/0".$request->id.") và chuyển sang thành hợp đồng chính thức";
                $nhatKy->save();
                ...
                $his->id_user = Auth::user()->id;
                $his->ngay = Date("H:m:s d-m-Y");
                $his->noiDung = "Gán xe: ".$car->typeCarDetail->name." VIN: " .$sokhung. "  màu: ".$mau." cho hợp đồng chờ và chuyển sang thành hợp đồng chính thức";
                $his->ghiChu = "";
                $his->save(); 
    ...
    public function yeuCauHuy(Request $request){
        ...
        if($result) {
            ...
            $nhatKy->thoiGian = Date("H:m:s");
            $nhatKy->ghiChu = Carbon::now();
            $nhatKy->noiDung = "Gửi yêu cầu hủy hợp đồng " . $code . " (Mã đề nghị ĐN/0".$request->idRequestHuy.") lý do: " . $request->lyDoHuy;
            $nhatKy->save();
    ...
    public function duyetYeuCauSua(Request $request){
        ...
        if(Auth::user()->hasRole('adminsale') || Auth::user()->hasRole('system')) {
            ...
            $result->code = 0; 
                if ($result->id_car_kho != null && $result->hdWait != true) {
                    ...
                    $car->type = "STORE";
                        ...
                        $nhatKy->ghiChu = Carbon::now();
                        $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                        $nhatKy->noiDung = "Yêu cầu sale chỉnh sửa đề nghị thực hiện hợp đồng (Mã đề nghị ĐN/0".$request->id.") Chuyển xe ".$car->typeCarDetail->name." vào kho và trạng thái xe thành STORE";
                        $nhatKy->save();
                        $his = new HistoryHopDong();
                        $his->idDeNghi = $request->id;
                        $his->id_user = Auth::user()->id;
            ...
            if($result) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                $nhatKy->noiDung = "[Quyền Admin] Thực hiện cho phép sale sửa đề nghị thực hiện hợp đồng (Mã đề nghị ĐN/0".$request->id.") <br/>Lý do chỉnh sửa: " . $temp;
                $nhatKy->save();
                $his = new HistoryHopDong();
                $his->idDeNghi = $request->id;
                $his->id_user = Auth::user()->id;
    ...
    public function duyetYeuCauSuaLead(Request $request){
        ...
        if(Auth::user()->hasRole('tpkd') || Auth::user()->hasRole('system')) {
            ...
            $result->code = 0;
                if ($result->id_car_kho != null && $result->hdWait != true) {
                    ...
                    if ($car->xuatXe == true)
                        ...
                        $nhatKy->ghiChu = Carbon::now();
                        $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                        $nhatKy->noiDung = "Trưởng phòng từ chối phê duyệt hợp đồng từ Admin sale <br/>Lý do từ chối:" . $lyDo . " (Mã đề nghị ĐN/0".$request->id.") Chuyển xe ".$car->typeCarDetail->name." vào kho và trạng thái xe thành STORE";
                        $nhatKy->save();
                        $his = new HistoryHopDong();
                        $his->idDeNghi = $request->id;
                        $his->id_user = Auth::user()->id;
            ...
            if($result) {
                ...
                $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->noiDung = "Trưởng phòng thực hiện duyệt yêu cầu sửa hợp đồng " . $code . " (Mã đề nghị ĐN/0".$request->id.") <br/>Lý do chỉnh sửa: " . $lyDo;
                $nhatKy->save();
                $his = new HistoryHopDong();
                $his->idDeNghi = $request->id;
                $his->id_user = Auth::user()->id;
    ...
    public function huyDeNghi(Request $request){
        ...
        if(Auth::user()->hasRole('adminsale') || Auth::user()->hasRole('system')) {
            ...
            if($result) {
                ...
                $nhatKy->ghiChu = Carbon::now();
                $nhatKy->chucNang = "Kinh doanh - Phê duyệt đề nghị";
                $nhatKy->noiDung = "Từ chối phê duyệt đề nghị thực hiện hợp đồng (Mã đề nghị ĐN/0".$request->id.")";
                $nhatKy->save();
        ...
        $templateProcessor = new TemplateProcessor('template/PHUKIEN.docx');
            ...
            foreach($package as $row) {
                if ($row->type == 'pay') {
                    ...
                    $sttpkban .= $i . '<w:br/>';
                    $chietKhauBlank .= '........ <w:br/>';          
                    ($saleOff->giamGia != 0) ? $chietKhau .= $saleOff->giamGia.'% <w:br/>' : $chietKhau .= " <w:br/>";                                
                    $pkbansl .= '1 <w:br/>';
                    $pkban .= $row->name . '<w:br/>';
                    $pkbangia .= number_format($row->cost) . '<w:br/>';
                    $pkbangiathanhtien .= number_format($row->cost - ($row->cost*$saleOff->giamGia/100)) . '<w:br/>';
                    $i++;
                    $tonggiaban += ($row->cost - ($row->cost*$saleOff->giamGia/100));
                }
    ...
    public function baoCaoHopDong(Request $request){
        ...
        foreach($hd as $row) {
            ...
            &&  (strtotime(\HelpFunction::getDateRevertCreatedAt($row->created_at)) <= strtotime($_to))) {
                ...
                $giaVonXeTax = 0;
                $giaBanXeTax = 0;
                if (!empty($row->carSale->typeCar->name) 
                    && stripos($row->carSale->typeCar->name, 'STARIA') !== false) {
                    $giaBanXeTax = $giaXe / 1.08;
                } else {
                    $giaBanXeTax = $giaXe / 1.1;
                }
                $congDKTax = 0;
                $phuKienBanTax = 0;
                $phuKienTangTax = 0;
                ...
                foreach($package as $row2) {
                    ...
                    if ($row2->type == 'pay') {
                        ...
                        $phuKienBanTax += ($row2->cost - ($row2->cost*$saleOff->giamGia/100)) * 0.4 / 1.08; 
                ...
                echo "<tr>
                    ...
                    <td class='text-bold'><span>".number_format($loiNhuan)."</span> <span class='text-success'>(".number_format($loiNhuanTamTinh).")</span></td>
                    <td>".$tiSuat." <span class='text-success'>(".$tiSuatTamTinh.")</span></td>
    ...
    public function exportExcel($from,$to,$loai) {
        ...
        $nhatKy->ghiChu = Carbon::now();
        $nhatKy->chucNang = "KINH DOANH - BÁO CÁO HỢP ĐỒNG";
        $nhatKy->noiDung = "Xuất báo cáo hợp đồng ra file excel Từ ". $from . " đến " . $to;
        $nhatKy->save();
    ...
    public function loadChiTietHopDong(Request $request){
        ...
        //--------
        $phuKien = SaleOffV2::select('package.*','saleoffv2.giamGia')
        ->join('packagev2 as package','saleoffv2.id_bh_pk_package','=','package.id')
        ->join('hop_dong as h','saleoffv2.id_hd','=','h.id')
        ->where([
            ['saleoffv2.id_hd','=', $hd->id],
    ...
    public function moveHD(Request $request) {
        ...
        if (Auth::user()->hasRole("system")) {
            ...
            $nhatKy->ghiChu = Carbon::now();
            $nhatKy->chucNang = "KINH DOANH - PHÊ DUYỆT HỢP ĐỒNG";
            $nhatKy->noiDung = "Chuyển thông tin hợp đồng cho cho nhân viên: ".$user->userDetail->surname." (Đã bao gồm chuyển khách hàng: ".$guest->name.")";
            $nhatKy->save();
